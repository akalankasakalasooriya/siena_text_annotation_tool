    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            entities.forEach(function (item) {
                text += `<li class="list-options entity-list" data-entity-name="${item.ENTITY_NAME}" data-entity-replacer="${item.ENTITY_REPLACER}" data-entity-color="${item.ENTITY_COLOR}"><p> ${item.ENTITY_NAME} <span style="color:${item.ENTITY_COLOR};">•</span></p></li>`;
            });
            $('#siena-entity-list').html(text);

        },
        error: function (err) {
            $('#siena-entity-list').html("Error");
        }
    });
...
$(document).on('click', `#siena-manage-entities-btn-update`, function () {
    ...
    if (!entity_name && !entity_replacer && !entity_color) {
        ...
        text = `<li class="list-options entity-list" data-entity-name="${entity_name}" data-entity-replacer="${entity_replacer}" data-entity-color="${entity_color}"><p> ${entity_name} <span style="color:${entity_color};">•</span></p><li>`;
